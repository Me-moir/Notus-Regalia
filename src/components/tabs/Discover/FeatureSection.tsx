...
const FeatureSection = memo(({
  ...
  const swipeRef = useRef({
    ...
    currentX: 0,
    isDragging: false,
    didMove: false,           // true once we decide the gesture is horizontal
    baseTranslateX: 0,        // the container's translate at drag-start
  });
  const [swipeOffset, setSwipeOffset] = useState(0); // live px offset during drag
  ...
  // 🔥 Mouse tracking for left panel - VIBRANT GRADIENTS (DESKTOP ONLY)
  useEffect(() => {
    const leftPanel = leftPanelRef.current;
    if (!leftPanel || isMobile) return; // Skip on mobile
  ...
  useEffect(() => {
    ...
    cardPositionTimeoutRef.current = requestAnimationFrame(() => {
      ...
      if (isMobile) {
        // If the user is mid-swipe, don't fight them — let swipeOffset drive position
        if (swipeOffset !== 0) return;

        const cardWidth = 280; // mobile card width
        const gap = 40;        // mobile gap (gap-10)
        const containerWidth = window.innerWidth - 32; // px-4 = 16px each side
        ...
        // --- DESKTOP layout centering (unchanged) ---
        const cardWidth = 400;
        const gap = 80;
  ...
  // Returns the translateX (px) that the container should sit at for a given 1-based card index.
  const getMobileBaseX = useCallback((cardIdx: number) => {
    const cardWidth = 280;
    const gap = 40;
    const containerWidth = window.innerWidth - 32; // px-4 = 16 each side
    const centerPosition = containerWidth / 2;
  ...
  const handleSwipeMove = useCallback((e: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (!swipeRef.current.didMove) {
      ...
      if (Math.abs(dy) > Math.abs(dx)) {
        // Vertical scroll wins → abort swipe so page scrolls normally
  ...
  const handleSwipeEnd = useCallback(() => {
    ...
    // Reset offset — the cardPosition effect will snap the container to the correct spot
    setSwipeOffset(0);
  }, [isMobile, activeCardIndex, goToCard]);

  // ─── LIVE DRAG TRANSLATION (mobile) ─────────────────────────────────
  // While the finger is down and swipeOffset is non-zero we push the
  // translate directly onto the container with transition:none so it
  // tracks the finger at 60 fps without fighting the snap effect.
  useEffect(() => {
      ...
      {allProjects.map((project, index) => {
        ...
        return (
          <div
            ...
            onMouseMove={!isMobile && isActive ? (e) => handleCardMouseMove(e, index) : undefined}
            onMouseLeave={!isMobile && isActive ? () => handleCardMouseLeave(index) : undefined}
            style={{ 
  ...
  return (
    <section 
      ...
      style={{
        background: 'linear-gradient(to bottom, rgb(20, 20, 20) 0%, rgb(10, 10, 10) 50%, rgb(0, 0, 0) 100%)',
        minHeight: 'max(100vh, 900px)', // Ensures section is tall enough for all content
        paddingTop: '10vh',
        paddingBottom: '20vh',
        borderTop: '1px dashed rgba(255, 255, 255, 0.2)',
        borderBottom: '1px dashed rgba(255, 255, 255, 0.2)'
